...
type PhotoDraft = { data: string; contentType: string; fileName: string; preview: string };
...
function ChallengeForm() {
  ...
  const create = trpc.challenges.create.useMutation({ onSuccess: async result => { await utils.challenges.list.invalidate(); await utils.challenges.mine.invalidate(); toast.success("Your challenge is now visible to the network."); setLocation(`/challenge/${result.id}`); }, onError: error => toast.error(error.message) });
  if (!user) return null;
