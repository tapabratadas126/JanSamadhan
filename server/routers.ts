...
// The browser uploads the image directly to Vercel Blob (see
// server/blobUpload.ts + client/src/lib/uploadPhoto.ts) and only sends us
// the resulting public URL and blob pathname — the image bytes never pass
// through this API.
...
export const appRouter = router({
  ...
  challenges: router({
    ...
    create: citizenProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Reject any photo that isn't actually this user's own Blob upload —
        // the client can only have gotten a valid token for its own path
        // (enforced in server/blobUpload.ts), so this also guards against a
        // tampered request pointing at someone else's blob.
        for (const photo of input.photos) {
          if (photo.fileKey && !photo.fileKey.startsWith(`challenge-photos/${ctx.user.id}/`)) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Invalid photo upload." });
          }
        }
